...
@Service
@Slf4j
public class ChnCardLandingFill implements CardLandingFill {
    @Resource
    ...
    public void fill(Map<String, Object> map, ZCardEntity zCardEntity, ZChargeEntity chargeEntity, SysDeptEntity deptEntity, SysUserEntity merchant) {
        // todo
        if (chargeEntity.getPayCode().equals(ZooConstant.CHN_PAYCODE_YSF) ) {
            this.ysfFill(map, zCardEntity, chargeEntity, deptEntity, merchant);
            return;
        }
        // todo
        if (chargeEntity.getPayCode().equals(ZooConstant.CHN_PAYCODE_ALIPAY) ) {
            this.ysfFill(map, zCardEntity, chargeEntity, deptEntity, merchant);
            return;
        }
        // todo
        if (chargeEntity.getPayCode().equals(ZooConstant.CHN_PAYCODE_WECHAT) ) {
            this.ysfFill(map, zCardEntity, chargeEntity, deptEntity, merchant);
            return;
        }
        // todo
        if (chargeEntity.getPayCode().equals(ZooConstant.CHN_PAYCODE_BANK) ) {
            this.ysfFill(map, zCardEntity, chargeEntity, deptEntity, merchant);
            return;
        }
     ...
     * @param zCardEntity
     */
    private void ysfFill(Map<String, Object> map, ZCardEntity zCardEntity, ZChargeEntity chargeEntity, SysDeptEntity deptEntity, SysUserEntity merchant) {
