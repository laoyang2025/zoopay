<template>
  <div class="mod-${backendUrl}__${classname}">
  ...
  <#if column.list>
    ...
    <#else>
      ...
      <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
        <template v-slot="scope">
          <el-button v-if="state.hasPermission('${backendUrl}:${classname}:update')" type="primary" link @click="addOrUpdateHandle(scope.row.id)">{{ $t("update") }}</el-button>
          <el-button v-if="state.hasPermission('${backendUrl}:${classname}:delete')" type="primary" link @click="state.deleteHandle(scope.row.id)">{{ $t("delete") }}</el-button>
        </template>
      </el-table-column>
    </el-table>
...
const view = reactive({
  getDataListURL: "/${backendUrl}/${classname}/page",
  getDataListIsPage: true,
  exportURL: "/${backendUrl}/${classname}/export",
  deleteURL: "/${backendUrl}/${classname}",
  deleteIsBatch: true,
  <#list columnList as column>
  <#if column.query>
